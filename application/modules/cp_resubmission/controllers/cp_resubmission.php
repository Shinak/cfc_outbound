...
class Cp_resubmission extends MX_Controller
{
    ...
    function get_resbmission_list($project_id, $typ = 'all', $ownership = 'all')
    {
        ...
        $sql = 'Select * from cp_resubmission as resub inner join company as comp where resub.company_id = comp.id and resub.project_id = '.$project_id.' '.$sql_typ.' '.$sql_user.' order by date desc, time desc';
    ...
    function set_resubmission($data)
    {
        ...
        $check = $this->check_resubmission($data['company_id']);
        if($check == FALSE)
        { 
            $this->_insert($data);
            $msg_part = "set a";
        ...
        {
            $id = $this->get_resub_id($data['company_id']);
        ...
        Modules::run('customer/_update_resub', $data['company_id'], 1);
    ...
    function unset_resubmission($company_id)
    {

        $id = $this->get_resub_id($company_id);
        $query = $this->get_where($id);
        $this->_delete($id);
        $data['resubmission'] = 0;
        Modules::run('customer/_update', $company_id, $data);
        ...
        if($date_time < $sql_date_time)
        {
            ...
            $info_data['info_typ'] = 'user_change';
            $info_data['company_id'] = $company_id;
            Modules::run('cp_info/save_info', $info_data);
        }  
    }
    ...
    function count_resub($project_id, $how = 'self')
    {
        ...
        $query_all = $this->_custom_query('Select * from cp_resubmission where project_id = '.$project_id.' '.$sql_stat.'');
        $all_resub = $query_all->num_rows();

        //Resubmission Ready
        $query_ready = $this->_custom_query('Select * from cp_resubmission where project_id = '.$project_id.' '.$sql_stat.' and ( date < "'.$date.'" or  date = "'.$date.'" and time < "'.$time.'")');
    ...
    function count_other($project_id)
    {
        ...
        $query_all = $this->_custom_query('Select * from cp_resubmission where project_id = '.$project_id.' and user_id != '.$user_id);
        $all_resub = $query_all->num_rows();

        //Resubmission by others Ready
        $query_ready = $this->_custom_query('Select * from cp_resubmission where project_id = '.$project_id.' and user_id != '.$user_id.' and time > '.$time.' and date >= '.$date);
    ...
    function check_resubmission($company_id)
    {
        $query = $this->get_where_custom('company_id', $company_id);
        if($query->num_rows() > 0)
        {
            $return = TRUE;
        }
    ...
    function get_resub_id($company_id)
    {
        $query = $this->get_where_custom('company_id', $company_id);
        foreach ($query->result() as $row)
        {
    ...
    function get_resub_data($company_id)
    {
        $id = $this->get_resub_id($company_id);
        $query = $this->get_where($id);
        foreach ($query->result() as $row)
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $query = $this->mdl_cp_resubmission->get($order_by);
        return $query;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $query = $this->mdl_cp_resubmission->get_with_limit($limit, $offset, $order_by);
        return $query;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $query = $this->mdl_cp_resubmission->get_where($id);
        return $query;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $query = $this->mdl_cp_resubmission->get_where_custom($col, $value);
        return $query;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $this->mdl_cp_resubmission->_insert($data);
    }

    function _update($id, $data) 
    {
        $this->load->model('mdl_cp_resubmission');
        $this->mdl_cp_resubmission->_update($id, $data);
    }

    function _delete($id) 
    {
        $this->load->model('mdl_cp_resubmission');
        $this->mdl_cp_resubmission->_delete($id);
    }

    function count_where($column, $value) 
    {
        $this->load->model('mdl_cp_resubmission');
        $count = $this->mdl_cp_resubmission->count_where($column, $value);
        return $count;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $max_id = $this->mdl_cp_resubmission->get_max();
        return $max_id;
    }
    ...
    {
        $this->load->model('mdl_cp_resubmission');
        $query = $this->mdl_cp_resubmission->_custom_query($mysql_query);
        return $query;
    }
